...
function setup() {
    ...
    d3.json(date_chosen == "" ? "static/js/taxi/NYCdata3.json" : ("taxi/data/" + date_chosen))
        .then((d = []) => {
...
function drawData(data, toRemove) {
    ...
    linePath = trips.enter()
        ...
        .each(function(d) {
            ...
            if (points.toString() == "764,302,361,-5294") {
                // console.log(d3.select(this))
                // d3.select(this).remove()
                console.log("path drawn : " + d.geometry)
            }
            d3.select(this).attr("stroke-dasharray", function() {
...
function queryData(newDate, oldDate = null) {
    ...
    return data_.filter(function(item) {
        if (date_chosen == "") {
            ...
            if (oldDate != null) {
                if (((tripDate.getDate() == newDate.getDate() &&
                            tripDate.getHours() <= newDate.getHours()) ||
                        (tripDate.getDate() < newDate.getDate())) &&
                    ((tripDate.getDate() == oldDate.getDate() &&
                            tripDate.getHours() > oldDate.getHours()) ||
                        (tripDate.getDate() > oldDate.getDate()))) {
                    return item;
                }
            } else {
...
function initTimeLine() {
    ...
    let myRangeslide = rangeslide("#rangeslide", {
        data: daysOfMonth.length == 0 ? hoursOfTheDay : daysOfMonth,
        showTicks: false,
        showValue: true,
        valuePosition: "above",
        valueIndicatorWidth: 167,
        showLabels: false,
        startPosition: 0,
        thumbHeight: 30,
        thumbWidth: 30,
