    ...
    return data_.filter(function(item) {
        if (date_chosen == "") {
            ...
            if (tripDate.getDate() == date.getDate() &&
                tripDate.getHours() == date.getHours()) { return item }

        } else {
            tripDate = new Date(item.properties[" dropoff_datetime"]);
            return (tripDate.getHours() == date.getHours());
        }
    });
}
...
function initTimeLine() {
    ...
    const changeValue = (d) => {
        ...
        document.querySelector("#rangeSliderValue").textContent = date;
        return date
    }
    ...
    if (date_chosen != "") {
        for (i = 0; i < 24; i++) {
            hoursOfTheDay.push([{
                value: new Date(date_chosen).addHours(i)
            }]);
        }
    } else {
    ...
    // console.log(datassd = daysOfMonth)
    let myRangeslide;
    ...
    options = {
        ...
        showValue: true,
        valuePosition: "above",
        valueSource: "function(data) { return changeValue(data.item) }",
        showLabels: false,
        startPosition: 0,
        thumbHeight: 30,
        thumbWidth: 30,
        ...
        handlers: {
            valueChanged: [function(data, element) {
                drawData(queryData(new Date(data.item)));
    ...
    myRangeslide = rangeslide("#rangeslide", options);
